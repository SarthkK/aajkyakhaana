...
import { planEntries, dishes, householdMembers, users, profiles } from "@/lib/db/schema";
import {
...
export type DaySummary = {
  date: string;
  /** What one person eats if they have a serving of everything planned. */
  perPerson: Nutrition;
  dishCount: number;
...
/**
 * Nutrition for one day, assuming each person eats one serving of every dish planned.
 * That is deliberately simple — it is a planning signal, not a food diary.
 */
export async function summarizeDay(householdId: string, date: string): Promise<DaySummary> {
  const rows = await db
    .select({ nutrition: dishes.nutrition, status: planEntries.status })
    .from(planEntries)
